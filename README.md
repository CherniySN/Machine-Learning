...
# Логистическая регрессия и ее интерпретация.

Предположим, что мы работаем в банке и нам необходимо спрогнозировать дефлот клиент, т.е. просрочки платежей.

Это задача бинарной классификации, и у клиента есть два состояния: дефлот - 1 и недофлт - 0.

Для практики мы будет использовать датасет с Kaggle:
https://www.kaggle.com/datasets/mariosfish/default-of-credit-card-clients

1) Скачаем и загрузим датасет в блокнот Jupiter Notebook;
2) Ознакомимся с блокнотом;
3) Проведем EDA;
4) Посмотрим как соотносится целевая переменная (дефолт\недефолт);
5) Посчитаем коэфициент дефолта (вероятность дефолта) - назовем ее базовой верятностью;
6) Расмотрим вероятность дефолта в разрезе категориальных данных и выявим категории 
которые влияют на дефолт;
7) Посмотрим на взаимную инфомацию - метод из теории информации, который показывает 
сколько информации мы сможем получить О целевой переменной зная информацию о категориальном признаке;
8) Обучим логистическую регрессию и сделаем прогнозы;
9) Интерпретируем логистическую регрессию.

Что бы понять логистическую регрессию нужно сначала посмотреть на формулу линейной регрессии.

![Картинка](https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQf7ySvBg1KI5GIt3IYNH7mi1XmIFzJhqrjvw&s)

Если мы помести результат полученный от формулы линейной регресси в сигмоиду - получим бинарную классификацию.

![Картинка](https://upload.wikimedia.org/wikipedia/commons/a/ac/Logistic-curve.png)
